...
class RendererKivy(RendererBase):
    ...
    def draw_path_collection(self, gc, master_transform, paths, all_transforms, offsets,
        ...
        uses_per_path = self._iter_collection_uses_per_path(paths, all_transforms, offsets,
        ...
        if not should_do_optimization:
            return RendererBase.draw_path_collection(self, gc, master_transform, paths,
        ...
        for xo, yo, path_poly, gc0, rgbFace in self._iter_collection(
                gc, master_transform, all_transforms, path_codes, offsets, offsetTrans, facecolors,
        ...
        for polygon in polygons:
            ...
            if newclip > -1:
                ...
                instructions_list.append(
                    (self.widget, self.get_graphics(gc, tess, points_line, rgbFace, closed=closed)))
            ...
            if len(polygons.meshes) != 0:
                ...
                for vertices, indices in polygons.meshes:
                    instruction_group.add(
                        Mesh(vertices=vertices, indices=indices, mode=str("triangle_fan")))
    ...
    def draw_mathtext(self, gc, x, y, s, prop, angle):
        ...
        texture = Texture.create(size=(w, h))
        if _mpl_ge_1_5:
            texture.blit_buffer(ftimage.as_rgba_str()[0][0], colorfmt='rgba', bufferfmt='ubyte')
        else:
            texture.blit_buffer(ftimage.as_rgba_str(), colorfmt='rgba', bufferfmt='ubyte')
    ...
    def draw_path(self, gc, path, transform, rgbFace=None):
        ...
        if _mpl_ge_2_0:
            ...
            polygons = path.to_polygons(transform, self.widget.width, self.widget.height)
        list_canvas_instruction = self.get_path_instructions(gc,
                                                             polygons,
                                                             closed=True,
                                                             rgbFace=rgbFace)
    ...
    def draw_markers(self, gc, marker_path, marker_trans, path, trans, rgbFace=None):
        ...
        if list_instructions is None:
            ...
            self._markers[dictkey] = self.get_path_instructions(gc, polygons, rgbFace=rgbFace)
    ...
    def get_text_width_height_descent(self, s, prop, ismath):
        ...
        if font is None:
            ...
            plot_text = CoreLabel(font_size=prop.get_size_in_points(), font_name=prop.get_name())
        plot_text.text = six.text_type("{}".format(s))
        plot_text.refresh()
...
class NavigationToolbar2Kivy(NavigationToolbar2):
    ...
    def _init_toolbar(self):
        ...
        for text, tooltip_text, image_file, callback in self.toolitems:
            ...
            if text in ['Pan', 'Zoom']:
                action_button = ActionToggleButton(text=text, icon=fname, group=id_group)
            else:
    ...
    def draw_rubberband(self, event, x0, y0, x1, y1):
        ...
        h = abs(y1 - y0)
        rect = [
            int(val) for val in (min(x0, x1) + self.canvas.x, min(y0, y1) + self.canvas.y, w, h)
        ]
        ...
        self.lastrect.add(Line(rectangle=rect, width=1.0, dash_length=5.0, dash_offset=5.0))
...
class FigureCanvasKivy(FocusBehavior, Widget, FigureCanvasBase):
    ...
    def keyboard_on_key_down(self, window, keycode, text, modifiers):
        ...
        return super(FigureCanvasKivy, self).keyboard_on_key_down(window, keycode, text, modifiers)
