...
class CalendarWidget(RelativeLayout):
    ...
    def init_ui(self):
        ...
        self.left_arrow = ArrowButton(text="<",
                                      on_press=self.go_prev,
                                      pos_hint={
                                          "top": 1,
                                          "left": 0
                                      })

        self.right_arrow = ArrowButton(text=">",
    ...
    def get_quarter(self):
        ...
        self.quarter_nums = cal_data.calc_quarter(self.active_date[2], self.active_date[1])
    ...
    def compare_day_name(self, dayname):
        return datetime.datetime.strptime(self.get_current_date(),
                                          '%d.%m.%Y').strftime('%A') == dayname

    # Gets Start, Mid or End and says if it is Start, End or Mid of the current month
    def compare_month_status(self, month_status):
        if month_status == 'Start' and self.active_date[0] == 1:
            return True
        elif month_status == 'End' and self.active_date[0] == calendar.monthrange(
                self.active_date[2], self.active_date[1])[1]:
            return True
        elif month_status == 'Mid' and self.active_date[0] == (
                calendar.monthrange(self.active_date[2], self.active_date[1])[1] // 2):
            return True
        else:
            return False
