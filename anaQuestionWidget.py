    ...
    def create_graph(self):
        ...
        if ana_type == 'none':
            ...
            for option in self.question_json['options']:
                x[option[1]] = 0
            for date in self.generate_data_range():
                # print(self.data[date][self.question_json['question'][0]])
                try:
                    no_value = True
            ...
            for date in self.generate_data_range():
                if self.question_json['options'][0][0] == 'exclusive option':
                    try:
                        for option, value in self.data[date][self.question_json['question']
                                                             [0]].items():
                            if value:
                                x.append(date)
                                y.append(int(option))
